...
### 4.8 Two separate evaluation suites, plus an ablation, instead of one bigger one
...
**Validated:** Both suites are runnable and reproducible in one command (`python -m eval.run_eval`), and `EVALUATION_SUMMARY.md` reports them side by side rather than blending them - see §5.
...
This codebase began as an AI-assisted first pass (Gemini/Antigravity) implementing the core three-tier pipeline, phonetic scoring, and an initial 16-case evaluation suite. It was then substantially reviewed, corrected, and extended with Claude before submission. Concretely, that second pass:

- **Found and fixed a real bug** in context-trigger extraction: a learned multi-word entity's own words (e.g. "pie", "torch" from "pie torch" → "PyTorch") were leaking into that entity's own trigger/negative-context list, which would have silently weakened negative-context vetoes for exactly the sentences they exist to catch (`tests/test_memory_engine.py::test_learn_from_correction_excludes_own_span_words_from_context`).
- **Found and fixed a real inconsistency**: the original `README.md` claimed latency numbers that directly contradicted the `eval_report.md` generated by the same codebase, and claimed a "conversational frequency reinforcement" learning source that had no corresponding implementation. Both are corrected in this revision (§5, §2).
- **Identified that the original 16-case evaluation suite was self-referential** (its entities were hand-seeded specifically to pass those 16 cases) and built the holdout generalization suite (§4.8) and ablation study (§4.4/§4.5) as the actual validation the brief asks for, including discovering and documenting a real failure mode (`HOLD-04`, §4.7) rather than only reporting cases chosen to succeed.
- **Designed and implemented the negative-evidence learning loop** (§4.7) as a new product capability, in direct response to the failure mode it discovered while building the holdout suite.
- **Fixed a real latency defect** (§4.6): the memory lookup was re-querying the database on every span of every transcript instead of once per transcript.

All product decisions, the specific test cases and their expected outcomes, and the interpretation of every result in this README were reviewed line-by-line before submission - nothing here is asserted without a corresponding runnable case in `eval/` or `tests/`.
